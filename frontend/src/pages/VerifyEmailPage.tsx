...
type State =
  { phase: 'verifying' } | { phase: 'success' } | { phase: 'error'; code: string; message: string };
...
export function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    void api('/api/auth/verify-email', { body: { token }, skipAuthRetry: true })
      .then(async () => {
      ...
      <main className={styles.stage}>
        ...
        <div className={styles.card}>
          {state.phase === 'verifying' && (
            <>
              <h1 className={styles.headline}>
                Verifying <span className={styles.headlineAccent}>Your Email</span>
              </h1>
              ...
              <CricketLoader label="Verifying your email…" size="block" />
            </>
          )}
          ...
          {state.phase === 'success' && (
            <>
              <h1 className={styles.headline}>
                Email <span className={styles.headlineAccent}>Verified!</span>
              </h1>
              <p className={styles.deck}>Welcome to NForce Arena. Your account is ready to go.</p>
          ...
          {state.phase === 'error' && (
            <>
              <h1 className={styles.headline}>
                Verification <span className={styles.headlineAccent}>Problem</span>
              </h1>
              ...
              {(state.code === 'TOKEN_EXPIRED' || state.code === 'TOKEN_ALREADY_USED') && (
                <p className={styles.deck} style={{ margin: '-8px 0 18px' }}>
                  Need a new link? Log in and use “resend verification”, or sign up again if you
                  never received one.
                </p>
              )}
