import { claimAccountSchema } from '@nforce/shared';
...
export function ClaimAccountPage() {
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault();
    setBanner(null);
    const check = validateForm(claimAccountSchema, { token, password });
    if (!check.ok) {
    ...
    setBusy(true);
    try {
      await api('/api/auth/claim-account', { body: check.data, skipAuthRetry: true });
      setDone(true);
    } catch (err) {
          ...
          {done ? (
            <>
              <h1 className={styles.headline}>
                Account <span className={styles.headlineAccent}>Ready</span>
              </h1>
              <p className={styles.deck}>
                Your own login is set up. Sign in with your new email and password whenever you're
                ready.
              </p>
            ...
            <>
              <h1 className={styles.headline}>
                Set Up Your <span className={styles.headlineAccent}>Own Login</span>
              </h1>
              <p className={styles.deck}>
                A parent or guardian invited you to take over your NForce Arena account. Choose a
                password to finish setting it up. Your history and registrations carry over exactly
                as they are.
              </p>
              ...
              <form onSubmit={(e) => void onSubmit(e)} noValidate>
                ...
                <button type="submit" className={styles.primary} disabled={busy}>
                  {busy ? <CricketLoader label="Setting up your account…" /> : 'Set up my account'}
                </button>
              </form>
            </>
          )}
